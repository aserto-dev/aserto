...
func getTenantID(ctx context.Context, c *cc.CommonCtx, tenantClient *tenant.Client, userIdentity, configPath string) (string, error) {
	resp, err := tenantClient.Account.GetAccount(ctx, &account.GetAccountRequest{})
	if err != nil {
...
func GetConnectionKeys(ctx context.Context, tenantClient *tenant.Client, token *keyring.TenantToken) error {
	ctx = client.SetTenantContext(ctx, token.TenantID)
	resp, err := tenantClient.Connections.ListConnections(
		ctx,
	...
	for _, cn := range resp.Results {
		...
		case cn.Kind == api.ProviderKind_PROVIDER_KIND_AUTHORIZER:
			if respX, err := GetConnection(ctx, tenantClient, cn.Id); err == nil {
...
func GetConnection(
	...
	return tenantClient.Connections.GetConnection(
		ctx,
